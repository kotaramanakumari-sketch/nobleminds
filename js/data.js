...
async function nmPromoteStudents(studentIds, targetYearId) {
  ...
  const clones = students.map(s => {
    // Increment Class
    let currentClass = s.class?.toString().trim();
    ...
    if (index !== -1) {
      ...
      // Fallback: try numeric increment if it's just a number not in list
      let num = parseInt(currentClass);
      if (!isNaN(num)) {
        nextClass = num === 12 ? 'Alumni' : (num + 1).toString();
      }
    }
